...
title    <- "Estimated second-wave underreporting factors associated with SARS-CoV-2 for states in India"       # change
subtitle <- "Based on data from April 1, 2020 till January 31, 2021."                                        # keep - unless the data is not through May 18 for whatever reason
...
p.bottom <-  ggplot(data.final %>% 
  ...
  geom_point(size=1, 
             shape=21, 
             aes(fill = Type)) + 
  geom_hline(yintercept = 13.3, color = "#3B4992FF", linetype = "dotted") + 
  geom_hline(yintercept = 3.46, color = "#EE0000FF", linetype = "dashed") + 
  ...
  annotate("text", label = glue("URF(C) for India\n13.3 (11.4 - 14.6)"),
           x = "Uttarakhand", y = 14.3, hjust = 0, vjust = -0.95, color = "#3B4992FF", 
           fontface = "bold", angle = 90, size = 5) +
  annotate("text", label = glue("URF(D) for India\n3.46 (3.13 - 3.69)"),
           x = "Uttarakhand", y = 4, hjust = 0, vjust = -0.95, color = "#EE0000FF", 
           fontface = "bold", angle = 90, size = 5) 
